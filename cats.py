...
def about(topic):
    ...
    def verify(para):
        for key in topic:
            for word in split(remove_punctuation(lower(para))):
                if key in word and len(key) == len(word):
                    return True  
        return False 
    return verify
    # END PROBLEM 2
    ...
    if typed == "" and reference == "":
        ...
        for i in range(min(len(typed_words), len(reference_words))):
            if typed_words[i] in reference_words[i]:
                acc_count += 1
    return acc_count / len(typed_words) * 100
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4 
    # get all characters (including spaces) in typed, divide characters by 5 to get word count, 60/elapsed = multipler for wpm, apply multiplier to the number of words
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    find_min_word = min(valid_words, key = lambda valid: diff_function(typed_word, valid, limit))
    if typed_word in valid_words or limit == 0:
        return typed_word 
    else:
        if diff_function(typed_word, find_min_word, limit) > limit:
            return typed_word
        else:
            return find_min_word 
...
def feline_flips(start, goal, limit):
    ...
    if start[0] == goal[0] and len(start) == 1 or limit < 0: 
        return 0 
    elif start[0] != goal[0] and len(start) == 1: 
        return 1    

    elif len(goal) > len(start):
        return feline_flips(start, goal[:len(goal)-len_diff], limit - len_diff) + len_diff
    elif len(goal) < len(start):
        return feline_flips(start[:len(start)-len_diff], goal, limit - len_diff) + len_diff
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    # BEGIN PROBLEM 8
    count = 0
    for word_i in range(len(sofar)):
        if sofar[word_i] == prompt[word_i]:
            count += 1
        else: 
            break
...
def time_per_word(words, times_per_player):
    ...
    for player in times_per_player:
        time_list = []
        for timestamp in range(1, len(player)):
            time_list += [player[timestamp] - player[timestamp - 1]]
        total_time_list.append(time_list)
    return match(words, total_time_list)
...
def fastest_words(match):
    ...
    for player in player_indices:
        fast_w_list.append(list())

    for word in word_indices:
        s = []
        for player in player_indices:
            s += [time(match, player, word)]
        fast_w_list[s.index(min(s))].append(word_at(match, word))
...
def match_string(match):
    """A corrections function that takes in a match object and returns a string representation of it"""
